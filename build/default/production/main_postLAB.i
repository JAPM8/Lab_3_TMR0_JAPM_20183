...
LOOP:
    ...
    BTFSC PORTB, 0 ; Se evalúa estado de ((PORTB) and 07Fh), 0 (si NO está presionado salta una línea)
    CALL INC_CONT ; Mientras se presione ((PORTB) and 07Fh), 0 pasa al antirebote de Incremento
    BTFSC PORTB, 1 ; Se evalúa estado de ((PORTB) and 07Fh), 1 (si NO está presionado salta una línea)
    CALL DEC_CONT ; Mientras se presione ((PORTB) and 07Fh), 1 pasa al antirebote de Decremento
...
INC_CONT:
    BTFSC PORTB, 0 ; Se evalúa estado de ((PORTB) and 07Fh), 0 (si NO está presionado salta una línea)
    GOTO $-1 ; Mientras se presione ((PORTB) and 07Fh), 0 se continúa revisando hasta que deje de estarlo
...
DEC_CONT:
    BTFSC PORTB, 1 ; Se evalúa estado de ((PORTB) and 07Fh), 1 (si NO está presionado salta una línea)
    GOTO $-1 ; Mientas se presione ((PORTB) and 07Fh), 1 se continua revisando hasta que deje de estarlo
...
CONT_TMR0:
    BTFSS ((INTCON) and 07Fh), 2 ; Verificación de interrupcion del TMR0
    ...
    BTFSC ((STATUS) and 07Fh), 2 ; Si la bandera está activa se hace el incremento de PORTD
    INCF PORTD
...
EST_DISP7:
    ...
    BTFSC ((STATUS) and 07Fh), 2 ; Si son iguales los contadores pasamos a una subrutina
    CALL BANDERA_IGUAL
